...
- Log: train.log stores metrics per epoch.
...
class OasisSlices(Dataset):
    ...
    def __getitem__(self, idx):
        img  = np.array(pil_open_gray(self.images[idx]))
        mask = np.array(pil_open_gray(self.masks[idx]))
...
def save_val_visuals(model: nn.Module, dl: DataLoader, cfg: Cfg, device: str, tag: str = "val"):
    ...
    for i in range(n_show):
        g = (img[i].float() / 255.0).clamp(0, 1)
        g_rgb = torch.cat([g, g, g], dim=0)
        p = colorize_mask(pred[i], cfg.num_classes).to(device)
        t = colorize_mask(mask[i], cfg.num_classes).to(device)
        tiles += [g_rgb, p, t]
...
def train(cfg: Cfg):
    ...
    # --- log file ---
    log_path = os.path.join(cfg.outputs, "train.log")
    with open(log_path, "w") as f:
        f.write("Epoch,TrainLoss,ValLoss,ValDice,PerClassDice\n")
    ...
    for ep in range(1, cfg.epochs + 1):
        ...
        with open(log_path, "a") as f:
            f.write(f"{ep},{tr_loss:.4f},{va_loss:.4f},{va_dsc:.4f},{per_class}\n")
...
def infer_single(cfg: Cfg, ckpt: str, image_path: str, save_path: str = None):
    ...
    img_np = np.array(pil_open_gray(image_path))
    img_t = torch.from_numpy(img_np).float().unsqueeze(0).unsqueeze(0)
    ...
    pred = logits.argmax(1).cpu().squeeze(0)
